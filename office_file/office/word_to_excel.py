...
def trans_to_excel(writer):
    ...
    for x in range (1,6):
        subset = pd.DataFrame(df[x])
        for i in range(1,subset.shape[1]):
            subset.iloc[0,i] = ''
        data = pd.concat([data,blank,subset], axis = 0, ignore_index = True)
    data = pd.concat([data1,data],axis = 0, ignore_index= True)
    data.to_excel(writer,index = False,sheet_name = file_name)
    print(data)
...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter('employee_data.xlsx')
    trans_to_html()
    for file in glob.glob('*.html'):
        trans_to_excel(writer)
        os.remove(file)
    writer.save()

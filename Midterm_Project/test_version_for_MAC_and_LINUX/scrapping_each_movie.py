...
def left_time_estimation(round_time,sleep_time,remain_workload):
    ...
    if  left_time < 3600:
        ...
        print('\nThis scrapping section will be finished in [ ' + time_estimation + ' ] minutes [ ' + seconds + ' ] seconds...(5/6)')
        ...
        print('\nThis scrapping section will be finished in [ ' + time_estimation + ' ] hours [ ' + mins + ' ] minutes [ ' + seconds + ' ] seconds...(5/6)')
        ...
        print('\nThis scrapping section will be finished in [ ' + time_estimation + ' ] day ' + '[ ' + hours + ' ] hours [ ' + mins + ' ] minutes [ ' + seconds + ' ] seconds...(5/6)')
    elif left_time >= 172800:
        time_estimation = str(math.modf(left_time/60/60/24)[1])
        hours = str(math.modf(math.modf(left_time/60/60/24)[0]*24)[1])
        mins = str(math.modf(math.modf(math.modf(left_time/60/60/24)[0]*24)[0]*60)[1])
        seconds = str(round(math.modf(math.modf(math.modf(left_time/60/60/24)[0]*24)[0]*60)[0]*60,2))
        print('\nThis scrapping section will be finished in [ ' + time_estimation + ' ] days ' + '[ ' + hours + ' ] hours [ ' + mins + ' ] minutes [ ' + seconds + ' ] seconds...(5/6)')
...
def pre_download(movie_link):
    ...
    file_names = movie_link.iloc[:,1].values
    for movie_order in range(len(file_names)):
        if movie_order != len(file_names)-1:
            file_name = file_names[movie_order]
            next_file = file_names[movie_order+1]
            link = links[movie_order]
            remain_work = len(file_names) - movie_order - 1

            if not os.path.exists('Each_Movie_Info/' + file_name + '.html'):
                download_movie_info(file_name,link,next_file,remain_workload = remain_work)
            else:
...
def download_movie_info(file_name, link, next_file,remain_workload):

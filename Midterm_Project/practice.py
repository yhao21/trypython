    ...
    def init_maingame(self):
        ...
        for col in range(1, 7):
            for row in range(1, 7):
                self.land = QPushButton()
                self.land.setObjectName('land %s-%s' % (row, col))
                self.land.setText('空地')
                self.land.setFont(QFont('楷体', 13))
                self.land.setFixedSize(80,80)
                self.MainGame_layout.addWidget(self.land, row, col, 1, 1)

                self.land.clicked.connect(self.new_building)

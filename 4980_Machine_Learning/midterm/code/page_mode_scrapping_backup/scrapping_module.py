...
class Scrapping():
    ...
    def __init__(self, url_list, folder_name, hours, page_mode = True, file_names = None):
        ...
        # default download page 1-5
        # if download deeplink, set page_mode = file_names (the list contain all file names)
        self.page_mode = page_mode
        self.DeepFile_names = file_names
    ...
    def check_file_existence(self):
        ...
        while self.count < self.repeat_times + 1:
            ...
            for url_order in self.url_list:
                ...
                for web_url in url_order:
                    ...
                    if self.page_mode == True:
                        ...
                        # pair order, i.e., nth (coin, gecko), n
                        fname_order = self.url_list.index(url_order)
                        # name order, i.e., first or second element in tuple (coin, gecko), e.g. coin for 0
                        fname_index = url_order.index(web_url)
                        self.file_name = self.DeepFile_names[fname_order][fname_index]
                    ...
                    if self.page_mode == True:
                        ...
                        if not os.path.exists(os.path.join(self.folder_path[fname_index], self.file_name + '.html')):
                            self.start_scrapping()
                        else:
                            print('\nFile' + self.output_format(website[which_web] + ': ' + self.file_name) + 'already exists!')
                            self.no_page_sleep = True
                            self.no_repetition_sleep = True
            ...
            if self.page_mode == True:
                ...
                self.count += 1
                # test
                print(self.no_repetition_sleep)
        ...
        if self.page_mode == True:
            ...
            print('\n' + '-' * 100)
            print('\nDone! You have downloaded all deeplink html files!')
    ...
    def start_scrapping(self):
        ...
        if self.page_mode == True:
            ...
            print('\nScrapping' + self.output_format(self.web_name + ': ' + self.file_name) \
                    + '...' * 20 + '(%d/%d)' % (self.workload - self.work_leftover + 1, self.workload))
